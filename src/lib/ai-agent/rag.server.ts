...
export async function answerQuestion(
  ...
  const systemPrompt =
    ...
    "When the answer is a multi-step process or a 'how do I…' task, ALSO include ONE Mermaid flowchart in a fenced ```mermaid code block that shows the steps clearly IN ORDER, A to Z. Use `flowchart TD`; give each step a short NUMBERED label (e.g. A[1 Open the Dispatch tab] --> B[2 Open History]); connect the steps top-to-bottom with arrows so the sequence is unmistakable; and label decision arrows Yes / No when a choice matters. Keep labels short and plain — letters, numbers and spaces only, no quotes, slashes, colons or other punctuation. Only use a `mindmap` when the user explicitly asks for an OVERVIEW of how things relate rather than an ordered sequence. Put a one or two sentence text explanation before the diagram and keep it to a handful of nodes. For a one-step answer, do not add a diagram. " +
