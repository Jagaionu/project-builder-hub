...
async function recomputeDayTotals(driverId: string, day: string) {
  const [{ data: legs }, { data: dwells }] = await Promise.all([
    supabaseAdmin
      .from("driving_legs" as never)
      .select("driving_minutes,from_warehouse_id")
      .eq("driver_id", driverId)
      .eq("leg_date", day),
    supabaseAdmin
      .from("stop_dwells" as never)
...
export const closeLeg = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    const { data: leg } = await supabaseAdmin
      .from("driving_legs" as never)
      .select("driver_id,leg_date,departed_at")
      .eq("id", data.legId)
      .maybeSingle();
    ...
    const driving_minutes = row.departed_at
      ? Math.max(
          0,
          Math.round(
            (new Date(data.arrivedAt).getTime() - new Date(row.departed_at).getTime()) / 60_000,
          ),
        )
      : 0;
    await supabaseAdmin
...
export const closeDwell = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    const { data: dwell } = await supabaseAdmin
      .from("stop_dwells" as never)
      .select("driver_id,dwell_date,arrived_at")
      .eq("id", data.dwellId)
      .maybeSingle();
    ...
    const dwell_minutes = row.arrived_at
      ? Math.max(
          0,
          Math.round(
            (new Date(data.departedAt).getTime() - new Date(row.arrived_at).getTime()) / 60_000,
          ),
        )
      : 0;
    await supabaseAdmin
