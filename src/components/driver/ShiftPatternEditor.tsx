...
export function ShiftPatternEditor({ driverId, initialDays, initialTimes, onSave }: ShiftPatternEditorProps) {
          ...
          {selectedInOrder.map((iso) => {
            ...
            return (
              <div key={iso} className="flex items-center gap-2">
                <span className="text-[11px] font-medium text-foreground w-20 shrink-0">
                  {dayName.slice(0, 3)}
                </span>
                <input
                  type="time"
                  ...
                  className="h-8 px-2 rounded border border-border bg-surface text-xs text-foreground focus:outline-none focus:ring-1 focus:ring-ring w-28"
