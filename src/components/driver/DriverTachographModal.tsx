...
export function DriverTachographModal() {
  ...
  const submit = async () => {
    ...
    if (drive <= 0) {
      toast.error("Enter your driving time for the week");
      return;
    }
    setBusy(true);
    try {
    ...
    <div className="fixed inset-0 z-[60] grid place-items-center bg-black/70 p-4" data-no-swipe>
      <div className="w-full max-w-sm box-border rounded-2xl border border-border bg-card p-5 shadow-2xl">
        ...
        <p className="mt-1 text-xs text-muted-foreground">
          Submit your real driving time for this week.
          {pending.length > 1 ? ` ${pending.length} weeks to confirm.` : ""}
        </p>
        <div className="mt-3 rounded-xl border border-border bg-background px-3 py-2 text-sm">
          <span className="text-muted-foreground">Week: </span>
          <span className="font-mono font-semibold text-foreground">
            {periodLabel(req.period_start, req.period_end)}
          </span>
        </div>
