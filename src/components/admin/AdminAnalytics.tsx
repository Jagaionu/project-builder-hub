      ...
      <div className="relative h-32">
        <ResponsiveContainer width="100%" height="100%">
          <PieChart>
            ...
            >
              {data.map((_, i) => (
                <Cell key={i} fill={empty ? "var(--surface-2)" : PALETTE[i % PALETTE.length]} />
              ))}
            </Pie>
...
function buildBuckets(days: number, g: Gran): { key: string; label: string }[] {
  ...
  if (g === "month") {
    for (let t = startOfMonth(since); t <= now; t.setMonth(t.getMonth() + 1)) {
      out.push({
        key: bucketKey(t, "month"),
        label: t.toLocaleDateString([], { month: "short", year: "2-digit" }),
      });
    }
  } else {
    const step = g === "day" ? 1 : 7;
    for (
      let t = g === "day" ? startOfDay(since) : startOfWeek(since);
      t <= now;
      t.setDate(t.getDate() + step)
    ) {
      out.push({
        key: bucketKey(t, g),
        label: t.toLocaleDateString([], { day: "2-digit", month: "short" }),
      });
    }
  }
  return out;
...
export function AdminAnalytics({
  ...
  const slicesFor = (pick: (u: Usage) => number) => {
    ...
    if (rest.length)
      top.push({ name: `+${rest.length} more`, value: rest.reduce((s, r) => s + r.value, 0) });
  ...
  useEffect(() => {
    ...
    void (async () => {
      const [jobs, drv, wh, tix] = await Promise.all([
        sb
          .from("jobs")
          .select("created_at,assigned_driver_id,planned_driver_id")
      ...
      for (const j of (jobs.data ?? []) as Array<{
        created_at: string;
        assigned_driver_id: string | null;
        planned_driver_id: string | null;
      }>) {
        const k = bucketKey(new Date(j.created_at), cfg.gran);
        const r = idx[k];
        if (!r) continue;
        r.vrids++;
        const did = j.assigned_driver_id ?? j.planned_driver_id;
        if (did) (active[k] ||= new Set()).add(did);
      }
      for (const k in active) if (idx[k]) idx[k].activeDrivers = active[k].size;
      for (const x of (drv.data ?? []) as Array<{ created_at: string }>) {
        const k = bucketKey(new Date(x.created_at), cfg.gran);
        if (idx[k]) idx[k].driversAdded++;
      }
      for (const x of (wh.data ?? []) as Array<{ created_at: string }>) {
        const k = bucketKey(new Date(x.created_at), cfg.gran);
        if (idx[k]) idx[k].warehousesAdded++;
      }
      for (const x of (tix.data ?? []) as Array<{ created_at: string }>) {
        const k = bucketKey(new Date(x.created_at), cfg.gran);
        if (idx[k]) idx[k].tickets++;
      }
  ...
  const totals = useMemo(
    () =>
      series.reduce(
        (a, r) => ({
          activeDrivers: Math.max(a.activeDrivers, r.activeDrivers),
          vrids: a.vrids + r.vrids,
          driversAdded: a.driversAdded + r.driversAdded,
          warehousesAdded: a.warehousesAdded + r.warehousesAdded,
          tickets: a.tickets + r.tickets,
        }),
        { activeDrivers: 0, vrids: 0, driversAdded: 0, warehousesAdded: 0, tickets: 0 },
      ),
    [series],
  );
    ...
    <div className="space-y-4">
      ...
      <div className="rounded-lg border border-border bg-surface p-4">
        <div className="flex flex-wrap items-center justify-between gap-3 mb-3">
          <div>
            ...
            <div className="text-[11px] text-muted-foreground">
              Usage over time — to gauge adoption &amp; pricing tier
            </div>
          </div>
          <div className="flex items-center gap-2">
            <select
              ...
              className="h-8 px-2 rounded-md border border-border bg-background text-xs max-w-[200px]"
            >
              {companies.map((c) => (
          ...
          {mounted ? (
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={series} margin={{ top: 4, right: 8, left: -16, bottom: 0 }}>
                ...
                <Line
                  type="monotone"
                  dataKey="warehousesAdded"
                  name="Warehouses added"
                  stroke="#f59e0b"
                  strokeWidth={2}
                  dot={false}
                  isAnimationActive={false}
        ...
        <div className="mt-2 flex flex-wrap gap-x-4 gap-y-1 text-[11px] text-muted-foreground">
          <span>
            Peak active drivers: <b className="text-foreground">{totals.activeDrivers}</b>
          </span>
          <span>
            VRIDs: <b className="text-foreground">{totals.vrids}</b>
          </span>
          <span>
            Drivers added: <b className="text-foreground">{totals.driversAdded}</b>
          </span>
          <span>
            Warehouses added: <b className="text-foreground">{totals.warehousesAdded}</b>
          </span>
          <span>
            Tickets: <b className="text-foreground">{totals.tickets}</b>
          </span>
