...
export function AdminSupportPanel({ companies }: { companies: Company[] }) {
  ...
  const load = useCallback(async () => {
    let q = sb
      ...
      .select(
        "id,ref,tenant_id,created_by_name,created_by_email,category,severity,title,description,status,attachments,admin_note,created_at",
      )
  ...
  useEffect(() => {
    const ch = supabase
      .channel("rt-support-admin-" + Math.random().toString(36).slice(2))
      .on(
        "postgres_changes",
      ...
      <div className="flex items-center gap-2">
        ...
        <div className="ml-auto relative">
          ...
          <input
            ...
            placeholder="Search company, title, ref, status…"
