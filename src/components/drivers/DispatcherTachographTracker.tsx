...
type Row = {
  ...
  status: string; submitted_at: string | null; discrepancy: boolean;
};
...
export function DispatcherTachographTracker({ drivers, onClose }: { drivers: Driver[]; onClose: () => void }) {
  ...
  const grouped = useMemo(() => {
    ...
    for (const r of rows) { const a = m.get(r.driver_id) ?? []; a.push(r); m.set(r.driver_id, a); }
    const out: { driverId: string; weeks: Row[] }[] = [];
    for (const [driverId, list] of m) {
      const weeks = [...list].sort((a, b) => b.period_start.localeCompare(a.period_start)).slice(0, 4);
      out.push({ driverId, weeks });
    }
    out.sort((a, b) => (nameById.get(a.driverId) ?? "").localeCompare(nameById.get(b.driverId) ?? ""));
    return out;
  }, [rows, nameById]);

  return (
    <div className="fixed inset-0 z-50 grid place-items-center bg-black/50 p-4" onClick={onClose}>
      <div className="w-full max-w-3xl max-h-[85vh] flex flex-col rounded-2xl border border-border bg-card shadow-2xl" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="flex-1 overflow-y-auto">
          <table className="w-full text-sm">
            <thead className="bg-surface text-[10px] font-mono uppercase tracking-widest text-muted-foreground sticky top-0">
              <tr><th className="px-4 py-2 text-left w-44">Driver</th><th className="px-4 py-2 text-left">Weeks</th></tr>
            </thead>
            <tbody className="divide-y divide-border">
                ...
                <tr key={g.driverId} className="hover:bg-surface-2/40 align-top">
                  ...
                  <td className="px-4 py-3">
                    <div className="flex flex-wrap gap-2">
                      {g.weeks.map((w) => (
                        <span key={w.id} className="inline-flex items-center gap-1.5 rounded-md border border-border bg-surface px-2 py-1 text-xs">
                          <span className="font-mono">{periodLabel(w.period_start, w.period_end)}</span>
                          {w.status === "submitted" ? (
                          ...
                          {w.discrepancy && <span className="text-warning" title="Submitted hours differ a lot from the GPS estimate">⚠</span>}
                          {w.status === "submitted" && (
                            <button onClick={() => resend(w.id)} className="ml-1 text-[10px] underline text-muted-foreground hover:text-foreground">resend</button>
                          )}
                        </span>
                      ))}
                    </div>
              ...
              {grouped.length === 0 && (
                <tr><td colSpan={2} className="px-4 py-8 text-center text-xs text-muted-foreground">No requests</td></tr>
              )}
            </tbody>
          </table>
        </div>
