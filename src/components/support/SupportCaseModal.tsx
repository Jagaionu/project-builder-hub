...
const CATEGORIES = [
  "GPS / Tracking",
  "Dispatch / Planning",
  "Route import (bulk upload)",
  "Drivers / Shifts / Holidays",
  "Tachograph / Hours",
  "Login / Access",
...
export function SupportCaseModal({ onClose }: { onClose: () => void }) {
  ...
  const load = useCallback(async () => {
    ...
    const { data } = await sb
      ...
      .select(
        "id,ref,severity,title,status,assigned_name,assigned_avatar,created_by_name,created_by_avatar,created_at,updated_at",
      )
  ...
  useEffect(() => {
    const ch = supabase
      .channel("rt-support-list-" + Math.random().toString(36).slice(2))
      .on(
        "postgres_changes",
  ...
  const addFiles = (list: FileList | null) => {
    if (list) setFiles((p) => [...p, ...Array.from(list)].slice(0, 6));
  };
  ...
  const submit = async () => {
    if (!tenantId) {
      toast.error("No company in context");
      return;
    }
    ...
    try {
      ...
      const { data, error } = await sb
        ...
        .insert({
          ...
          created_by_avatar: avatarUrl ?? null,
          category,
          severity,
          title: title.trim(),
  ...
  const Shell = ({ children }: { children: React.ReactNode }) => (
    ...
    >
      <div
        className="w-full max-w-4xl max-h-[88vh] flex flex-col box-border rounded-2xl border border-border bg-card shadow-2xl"
        ...
        <div className="p-5 overflow-y-auto">
          <div className="grid grid-cols-2 gap-3">
            <label className="block">
              <span className="block text-[11px] font-semibold uppercase tracking-widest text-muted-foreground mb-1">
                Category
              </span>
              <select
                value={category}
          ...
          <label className="block mt-3">
            ...
            <input
              value={company?.name ?? ""}
              readOnly
              className="w-full h-10 px-3 rounded-lg border border-border bg-surface text-sm text-muted-foreground"
            />
          </label>
          <label className="block mt-3">
            <span className="block text-[11px] font-semibold uppercase tracking-widest text-muted-foreground mb-1">
          ...
          <label className="block mt-3">
            ...
            <textarea
              ...
              className="w-full box-border p-3 rounded-lg border border-border bg-background text-sm"
            />
          </label>
          <div className="mt-3">
            <span className="block text-[11px] font-semibold uppercase tracking-widest text-muted-foreground mb-1">
            ...
            {files.length > 0 && (
              <ul className="mt-2 space-y-1">
                {files.map((f, i) => (
                  <li
                    key={i}
                    className="flex items-center justify-between text-xs rounded border border-border bg-surface px-2 py-1"
                  >
          ...
          <button
            onClick={submit}
            disabled={busy}
            className="mt-4 w-full bg-primary text-primary-foreground font-bold py-3 rounded-xl disabled:opacity-40 active:scale-[0.99] transition"
          >
    ...
    <Shell>
      <div className="flex items-center justify-between px-5 py-4 border-b border-border">
        ...
        <div className="flex items-center gap-2">
          <button
            onClick={() => setView("new")}
            className="inline-flex items-center gap-1.5 h-9 px-3 rounded-lg text-sm font-semibold text-white"
            ...
            {rows.map((t) => {
              ...
              return (
                ...
                >
                  ...
                  <td className="px-3 py-2.5">
                    {t.assigned_name ? (
                      <span className="inline-flex items-center gap-1.5">
                        <PersonAvatar name={t.assigned_name} url={t.assigned_avatar} size={18} />
                  ...
                  <td className="px-3 py-2.5">
                    <span className="inline-flex items-center gap-1.5">
                      <PersonAvatar name={t.created_by_name} url={t.created_by_avatar} size={18} />
                      <span className="text-xs">{t.created_by_name ?? "-"}</span>
                    </span>
                  </td>
            ...
            {rows.length === 0 && (
              <tr>
                <td colSpan={8} className="px-3 py-10 text-center text-xs text-muted-foreground">
                  No cases yet. Click New ticket to raise one.
                </td>
              </tr>
            )}
          </tbody>
