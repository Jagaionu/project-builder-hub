...
export function SupportTicketThread({
  ...
  const loadTicket = useCallback(async () => {
    const { data } = await sb
      ...
      .select("id,ref,title,category,severity,status,created_at,created_by_name,description")
      .eq("id", ticketId)
      .maybeSingle();
  ...
  const loadMsgs = useCallback(async () => {
    const { data } = await sb
      ...
      .select("id,author_name,author_avatar,is_admin,is_system,body,created_at")
  ...
  const setStatus = async (status: string, sysMsg: string) => {
    ...
    if (status === "in_progress") {
      patch.assigned_to = userId ?? null;
      patch.assigned_name = name ?? null;
      patch.assigned_avatar = avatarUrl ?? null;
    }
  ...
  const reopen = async () => {
    ...
    await post("Re-opened by the reporter - the issue is not resolved.", true);
    await loadTicket();
    await loadMsgs();
  };
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="flex items-start gap-2 pb-3 border-b border-border">
        {onBack && (
          <button onClick={onBack} className="p-1 rounded hover:bg-surface-2 text-muted-foreground">
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="text-[11px] text-muted-foreground mt-0.5">
            {ticket.category} - Sev{ticket.severity} {SEV[ticket.severity] ?? ""} - raised{" "}
      ...
      <div className="flex-1 overflow-y-auto py-3 space-y-2 min-h-[180px]">
        <div className="rounded-lg border border-border bg-surface p-3 text-sm whitespace-pre-wrap">
          {ticket.description}
        </div>
        {msgs.map((m) =>
          m.is_system ? (
            <div key={m.id} className="text-center text-[10px] text-muted-foreground py-1">
            ...
            <div
              key={m.id}
              className={
                "flex flex-col max-w-[80%] " + (m.is_admin ? "items-start" : "items-end ml-auto")
              }
            >
              <div
                className={
              ...
              <span className="flex items-center gap-1 text-[9px] text-muted-foreground mt-0.5">
                <PersonAvatar name={m.author_name} url={m.author_avatar} size={14} />
                {m.author_name ?? (m.is_admin ? "Support" : "You")} - {when(m.created_at)}
              </span>
            </div>
          ),
        )}
      ...
      <div className="pt-3 border-t border-border space-y-2">
        <div className="flex items-center gap-2">
          ...
          <button
            onClick={send}
            disabled={busy}
            className="h-10 px-3 rounded-lg bg-primary text-primary-foreground text-sm font-semibold disabled:opacity-40"
          >
          ...
          {isAdmin ? (
            <>
              {ticket.status === "pending" && (
                <button
                  onClick={() => setStatus("in_progress", "Picked up - now In progress.")}
                  className="px-3 py-1.5 rounded-md text-xs font-medium border border-border hover:bg-surface-2"
                >
                  Mark in progress
                </button>
              )}
              {ticket.status !== "resolved" && (
                <button
                  ...
                  className="px-3 py-1.5 rounded-md text-xs font-semibold bg-success text-white"
                >
                  Resolve ticket
                </button>
              )}
            ...
            ticket.status === "resolved" && (
              <button
                onClick={reopen}
                className="px-3 py-1.5 rounded-md text-xs font-semibold border border-destructive/50 text-destructive hover:bg-destructive/10"
              >
                Re-open (not fixed)
              </button>
            )
