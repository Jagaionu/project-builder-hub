...
export const ComplianceDot = memo(function ComplianceDot({
  ...
  const title = offShift
    ...
    : (c.issues[0]?.msg ??
      `OK · ${c.daily.toFixed(1)}/10 today · ${c.weekly.toFixed(1)}/56 this week`);
...
export const PlannedChip = memo(function PlannedChip({
  ...
  return (
    <div
      title={
        isChained
          ? "Chained follow-on assignment — part of a multi-route sequence"
          : "Planned follow-on assignment — not confirmed yet"
      }
    ...
    >
      <span
        className={`size-1 rounded-full ${isChained ? "bg-blue-500" : "bg-muted-foreground/60"}`}
      />
